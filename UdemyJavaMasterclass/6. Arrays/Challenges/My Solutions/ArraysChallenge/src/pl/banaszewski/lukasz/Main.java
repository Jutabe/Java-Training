    ...
    public static int[] sortIntegers(int[] intArray) {
        ...
        for(int i=1; i<=intArray.length; i++) {
            for(int j=i; j<=sortedArray.length; j++) {
                if(intArray[j-1] > sortedArray[i-1]) {
                    int currentMaxValue = intArray[j-1];
                    int previousMaxValue = sortedArray[i-1];
                    sortedArray[i-1] = currentMaxValue;
                    intArray[j-1] = previousMaxValue;
                }
            }
        }
